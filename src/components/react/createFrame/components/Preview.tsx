...
const url35= 'https://images.unsplash.com/photo-1554448326-fa887763def2?q=80&w=2666&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
export default function Preview({ logic, className }: PreviewProps) {
    const renderFrame = () => {
        switch (logic.frameType) {
            ...
            case "35":
                return <div style={{ padding: `10% 5%`, backgroundColor: logic.frameColor, position: 'relative', color: logic.bwFilter ? '#ddd' : '#ffc376', 
                  transform: `
    ${logic.portraitMode ? 'rotate(90deg)' : ''}
    ${logic.flip ? ' scaleX(-1)' : ''}
  `.trim(),
            }} 
                className={css.filmFrame}>
                    <span className={css.frameIndex}>
                        <span>{logic.frameIndex}</span>
                    </span>
                       <div className={css.sprocketsTop}>
        {Array.from({ length: 8 }).map((_, i) => (
          <span key={i} className={css.sprocket} />
        ))}
      </div>
                    <img src={url35} />
                    <span className={css.filmInfo}>{camelCaseToUpperSpaced(logic.filmStock)} {logic.filmSpeed}</span>
                    <div className={css.sprocketsBottom}>
        {Array.from({ length: 8 }).map((_, i) => (
          <span key={i} className={css.sprocket} />
        ))}
      </div>
